    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher par username ou email..." className="input-field bg-dark-surface border-dark-border text-white text-sm pl-9 py-2" />
              ...
              <div key={m.id} className="bg-dark-surface border border-dark-border rounded-card p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => setSelectedId(m.id)} className="p-1.5 text-gray-500 hover:text-white transition-colors">
                      <Eye size={16} />
                    </button>
...
function CreateMemberModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-dark-surface border border-dark-border rounded-card p-6 w-full max-w-md space-y-4">
        <h2 className="text-lg font-bold text-white">Creer un membre</h2>
        <div>
          <label className="block text-sm text-gray-400 mb-1">Email</label>
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input-field bg-dark-bg border-dark-border text-white" />
        </div>
        <div>
          <label className="block text-sm text-gray-400 mb-1">Username</label>
          <input value={username} onChange={(e) => setUsername(e.target.value)} className="input-field bg-dark-bg border-dark-border text-white" />
        </div>
        <div>
          <label className="block text-sm text-gray-400 mb-1">Mot de passe</label>
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="input-field bg-dark-bg border-dark-border text-white" />
        </div>
        <div className="flex items-center gap-3 pt-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-input border border-dark-border text-gray-400 hover:text-white text-sm font-medium transition-colors">Annuler</button>
