  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <select value={catFilter} onChange={(e) => { setCatFilter(e.target.value); setPage(0); }} className="input-field bg-dark-surface border-dark-border text-white text-sm w-auto py-2">
          <option value="all">Toutes categories</option>
          {CATEGORY_KEYS.map((k) => <option key={k} value={k}>{CATEGORIES[k as CategoryKey].label}</option>)}
        </select>
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value as StatusFilter); setPage(0); }} className="input-field bg-dark-surface border-dark-border text-white text-sm w-auto py-2">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="input-field bg-dark-surface border-dark-border text-white text-sm pl-9 py-2"
          />
          {search && (
      ...
      {loading ? (
        ...
        <>
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {establishments.map((e) => (
                  <tr key={e.id} className="border-b border-dark-border/50 hover:bg-dark-surface/50">
          ...
          <div className="md:hidden space-y-3">
            {establishments.map((e) => (
              <div key={e.id} className="bg-dark-surface border border-dark-border rounded-card p-4 space-y-3">
            ...
            <div className="flex items-center justify-center gap-2 pt-4">
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  key={i}
                  onClick={() => setPage(i)}
                  className={`w-8 h-8 rounded text-sm font-medium transition-colors ${page === i ? 'bg-primary text-white' : 'text-gray-500 hover:text-white hover:bg-dark-surface'}`}
                >
                  {i + 1}
                </button>
              ))}
